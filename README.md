...
## Access
- via [Github Page](https://mcc-mak.github.io/hko-timesheet/?userId=ccmak)

一個基於 Firebase 的員工工時記錄與請假管理系統，支援半日/全日請假、公眾假期自動標記、特別安排備註、**累計工時差值追蹤**、**URL 參數快速訪問**等功能。
...
您可以透過 URL 查詢參數直接指定要檢視的使用者、年份和月份：

| 參數 | 說明 | 範例 |
|------|------|------|
| `userId` | 指定使用者 ID (覆蓋匿名登入的 UID) | `?userId=staff_001` |
| `year` | 指定年份 (2025-2030) | `&year=2025` |
| `month` | 指定月份 (0-11，0=一月, 11=十二月) | `&month=6` |

**使用範例**：
```
# 檢視特定使用者的 2025年7月 (月份索引 6)
https://mcc-mak.github.io/hko-timesheet/?userId=staff_001&year=2025&month=6

# 檢視當前使用者的 2026年1月
https://mcc-mak.github.io/hko-timesheet/?year=2026&month=0

# 檢視特定使用者 (不指定年份月份，顯示當月)
https://mcc-mak.github.io/hko-timesheet/?userId=manager_01
```

**注意事項**：
- 月份索引：0 = 一月, 1 = 二月, ..., 11 = 十二月
- 若指定的年份/月份超出有效範圍，系統會自動調整到最接近的有效值
- `userId` 參數會覆蓋匿名登入產生的 UID，用於查看特定使用者的資料
...
- 能匿名讀寫 `timesheet/{UID}/*` 及 `offset/{UID}/*` 路徑
- `holiday` 為公開讀寫 (僅管理員可修改)
- 可透過 URL 參數 `userId` 指定要檢視的使用者資料
...
- **v1.2.0** (目前版本)
  - ✨ 新增 URL 查詢參數支援 (`userId`, `year`, `month`)
  - ✨ 支援透過 URL 直接指定使用者 ID
  - ✨ 支援分享特定使用者特定月份的連結
  - 🔧 優化 Firebase 初始化流程
...
**Q: 如何使用 URL 參數查看特定使用者的資料？**  
A: 在網址後加上 `?userId=使用者ID` 即可，例如：`https://mcc-mak.github.io/hko-timesheet/?userId=staff_001`。也可同時指定年份和月份：`?userId=staff_001&year=2025&month=6`。
